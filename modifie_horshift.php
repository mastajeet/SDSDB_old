...
if(isset($_GET['IDInstallation'])){
	...
	$MainOutput->openrow();
	$MainOutput->opencol('100%',7);
	$MainOutput->addlink('index.php?Section=Generate_Day&IDHoraire='.$Info2['IDHoraire'],'G�n�rer l\'horaire d\'une journ�e');
	$MainOutput->closecol();
	$MainOutput->closerow();
	...
	$CJour = array(0=>'Dimanche',1=>'Lundi',2=>'Mardi',3=>'Mercredi',4=>'Jeudi',5=>'Vendredi',6=>'Samedi');
			...
			foreach($Info as $v){
				...
				$MainOutput->addtexte($Start['G']."h".$Start['i']." � ".$End['G']."h".$End['i'], 'Titre');
