...
if(!$_GET['ToPrint']){
	...
	$MainOutput->addlink('index.php?Section=Display_Shit&Semaine='.$_GET['Semaine'].'&ToPrint=TRUE', '<img border=0 src=assets/buttons/b_print.png>','_BLANK');
}
...
if(!$_GET['ToPrint']){
	...
	$MainOutput->AddLink('index.php?Section=Display_Shit&Semaine='.get_last_sunday(1,$_GET['Semaine']),'Semaine Pr�c�dente');
	$MainOutput->addoutput('</div>',0,0);
	$MainOutput->CloseCol();
	$MainOutput->OpenCol('450',3);
	$MainOutput->addoutput('<div align=right>',0,0);
...
//	$Req3 = "SELECT IDShift, Jour, Start, End, Warn, Confirme, Assistant, shift.IDEmploye, Nom, Prenom, HName FROM shift LEFT JOIN employe on shift.IDEmploye = employe.IDEmploye WHERE IDInstallation=".$Rep[0]." AND Semaine=".$_GET['Semaine']." AND (shift.IDEmploye=0 OR Warn<>'') ORDER BY Jour ASC, Assistant ASC, Start ASC";
	$Req3 = "SELECT IDShift, Jour, Start, End, Warn, Confirme, Assistant, shift.IDEmploye
       FROM shift 
WHERE IDInstallation=".$Rep[0]." AND Semaine=".$_GET['Semaine']." AND (shift.IDEmploye=0 OR Warn<>'') ORDER BY Jour ASC, Assistant ASC, Start ASC";
