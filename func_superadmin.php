...
function batch_delete($SDate,$EDate,$IDInstallation,$WhereStr){
    $SQL = new sqlclass();
    $GenerateLogReq = "SELECT IDShift FROM shift WHERE IDInstallation=".$IDInstallation." AND Semaine+Jour*(24*3600)+Start>=".$SDate." AND Semaine+Jour*(24*3600)+Start<=".$EDate." ".$WhereStr;
    $SQL->Select($GenerateLogReq);
    while($Rep = $SQL->FetchArray()){
...
function modifie_facture_coteseq($ini,$fin){
	...
	if(count($Init)==2){
		...
		$SQL->SELECT($Req);
		if($SQL->NumRow()==0)
			return "Cote cibl�e non existante.";
		$Nom =  $SQL->FetchArray();
		$Req = "SELECT IDFacture FROM facture WHERE Cote='".$Fint[0]."' AND Sequence=".$Fint[1];
		$SQL->SELECT($Req);
		if($SQL->NumRow()<>0)
			return "Vous ne pouvez pas �craser une factur� qui existe d�j�";
		$Req = "UPDATE facture SET Cote='".$Fint[0]."', Sequence='".$Fint[1]."' WHERE Cote='".$Init[0]."' AND Sequence=".$Init[1];
		$SQL->Update($Req);
			return $Init[0]."-".$Init[1]." est devenue ".$Fint[0]."-".$Fint[1]." et sera acc�ssible sous le dossier de facturation de <a href=index.php?Cote=".$Fint[0]."><b>".$Nom[0]."</b></a>";
	}ELSE{
