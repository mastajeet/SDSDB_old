...
$_POST['FORMCote'] = $_POST['FORMGenerateCote'];
	...
	$InstallationArray = get_installation_bycote($_POST['FORMCote']);
		...
		$SQL->SELECT($Req);	
			...
			//TraitementDerniï¿½reSemaine
								
					
					$Req = "SELECT Start, End, Jour, shift.TXH, installation.Nom, shift.Assistant, Ferie FROM shift JOIN installation JOIN client on shift.IDInstallation = installation.IDInstallation AND client.IDClient = installation.IDClient WHERE shift.IDInstallation = ".$k." AND Semaine=".$CurrentWeek." AND Jour <= $LastWeekDayOfTheMonth ORDER BY installation.Nom ASC, Jour ASC, Assistant ASC, Start ASC";
					$SQL->SELECT($Req);
					$i =0;
					$Shift = array();
	...
	$MainOutput->AddTexte('Mois Courant','Titre');
 	$MainOutput->CloseCol();
	$MainOutput->CloseRow();
