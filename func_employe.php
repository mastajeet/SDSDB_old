	...
	$MainOutput->OpenRow();
		for($d=0;$d<=6;$d++){
			...
			while($Rep = $SQL->FetchArray()){
                ...
                if($Rep['Message']<>""){
                    $MainOutput->addtexte("<a title=\"".$Rep['Message']."\">".$Start['G']."h".$Start['i']."&nbsp;�&nbsp;".$End['G']."h".$End['i']."</a>", 'unconfirmed');
                }elseif($Rep['Warn']<>""){
                    $MainOutput->addtexte("<a title=\"Appelles nous svp!\">".$Start['G']."h".$Start['i']."&nbsp;�&nbsp;".$End['G']."h".$End['i'], 'unconfirmed');
                }else{

                    if($SQL2->NumRow()==0){
                        $MainOutput->addtexte($Start['G']."h".$Start['i']."&nbsp;�&nbsp;".$End['G']."h".$End['i'], 'ok');
                    }else{
                        $Rep2 = $SQL2->FetchArray();
                        if($Rep2[0]==0)
                                $MainOutput->addtexte($Start['G']."h".$Start['i']."&nbsp;�&nbsp;".$End['G']."h".$End['i'], 'torempl');
                        else{
                            if($Rep2[0]==$IDEmploye)
                                $MainOutput->addtexte($Start['G']."h".$Start['i']."&nbsp;�&nbsp;".$End['G']."h".$End['i'], 'ok');
                            else
...
function get_employe_horaire_email($IDEmploye,$Semaine){
	...
	border-style: solid;">',0,0);
			...
			if($Rep['Warn']<>"")
					$MainOutput->AddOutput("<font face=tahoma size=2 style='background-color: #FFD71C;'>".$Start['G']."h".$Start['i']."&nbsp;�&nbsp;".$End['G']."h".$End['i']."<font>",0,0);
		else{
			
			if($SQL2->NumRow()==0)
					$MainOutput->AddOutput("<font face=tahoma size=2 style='background-color: #0CFF00;'>".$Start['G']."h".$Start['i']."&nbsp;�&nbsp;".$End['G']."h".$End['i']."<font>",0,0);
	
			else{
				$Rep2 = $SQL2->FetchArray();
				if($Rep2[0]==0)
						$MainOutput->AddOutput("<font face=tahoma size=2 style='background-color: #FF72A4'>".$Start['G']."h".$Start['i']."&nbsp;�&nbsp;".$End['G']."h".$End['i']."<font>",0,0);
				else{
					if($Rep2[0]==$IDEmploye)
						$MainOutput->AddOutput("<font face=tahoma size=2 style='background-color: #0CFF00;'>".$Start['G']."h".$Start['i']."&nbsp;�&nbsp;".$End['G']."h".$End['i']."<font>",0,0);
					else
