...
//COLONE DE TITRE
	...
	$MainOutput->closecol();
		...
		$Req = "SELECT IDShift, Jour, Start, End, shift.IDEmploye, Nom, Prenom, HName FROM shift JOIN employe on shift.IDEmploye = employe.IDEmploye WHERE IDInstallation=".$Rep[0]." AND Semaine=".$_GET['Semaine']." ORDER BY Jour ASC, Assistant ASC, Start ASC";
		$SQL2->SELECT($Req);
		...
		while($Rep2 = $SQL2->FetchArray()){
			...
			if($Rep2['Assistant'] AND !$Assistant[$Rep2['Jour']]){
				...
				$Output[$Rep2['Jour']]->OpenCol();
					$Output[$Rep2['Jour']]->AddTexte('Assistant','Warn');
			...
			if($Rep2['HName']=="")
				...
				$Display = $Rep2['HName'];
			$Output[$Rep2['Jour']]->addlink('index.php?Section=Employe&IDEmploye='.$v['IDEmploye'],$Display);
			$Output[$Rep2['Jour']]->closecol();
			$Output[$Rep2['Jour']]->closerow();
			$Output[$Rep2['Jour']]->openrow();
			$Output[$Rep2['Jour']]->opencol();
