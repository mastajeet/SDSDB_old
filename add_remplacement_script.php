...
if(!isset($_POST['FORMEmail']))
	...
	$Rapport->AddTexte('Remplacements ajout�s � la liste','Titre');
	$Rapport->CloseCol();
	$Rapport->CloseRow();
	$RapportEmail .= "<table witdth=450 cellspacing=0 cellpadding=0><tr><td colspan=3 width=450><font face=tahoma size=2><b>Bonjour ".$InfoEmpl['Prenom']." (#".$_POST['FORMIDEmployeS']."), voici les remplacements que tu as demand�s</b></font><br>
	<br></td></tr>";
...
$AUCUN = TRUE;
	...
	$Rapport->OpenCol();
	$Rapport->addtexte($Start['G']."h".$Start['i']." � ".$End['G']."h".$End['i']);
	$Rapport->CloseCol();
	...
	$RapportEmail .= "<td width=150><font face=tahoma size=2>".$Start['G']."h".$Start['i']." � ".$End['G']."h".$End['i']."</font></td></tr>";
...
if($SQL->NumRow()<>0)
	$SQL2->QUERY($Req3);
		...
		$Rapport->AddTexte('Il manque quelques donn�es','Warning');
		$Rapport->CloseCol();
		$Rapport->CloseRow();
		$AUCUN=TRUE;
	}
...
$RapportEmail .= "<tr><td width=450 colspan=3><font face=tahoma size=2><b>Demand�:</b> ".$InfoEmpl2['Prenom']." ".$InfoEmpl2['Nom']."</font></td></tr>";
$RapportEmail .= "<tr><td width=450 colspan=3><font face=tahoma size=2><b>Demand� le:</b> ".$Time['d']."-".$Month[intval($Time['m'])]."-".$Time['Y']."</font></td></tr>";
$RapportEmail .= "<tr><td width=450 colspan=3><font face=tahoma size=2><b>Pour tous probl�mes, appelle nous au (418) 687-4047</font></b></td></tr></table>";
//Possibilit�� d'envoyer un email
if(!$AUCUN AND $_POST['FORMEmail'])
	send_mail($InfoEmpl['Email'],"Demande de remplacement re�ue",$RapportEmail,TRUE);
?>