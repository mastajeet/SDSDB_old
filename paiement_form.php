...
if(isset($_GET['ADD']) && $_GET['ADD']==TRUE){
	if(isset($current_cote)){
        ...
        $MainOutput->InputText('PayableYear','Ann�e de facturation',4,get_vars("BoniYear"));
...
$MainOutput->OpenCol('',4);
    $MainOutput->AddTexte(' ');
    $MainOutput->br();
    $MainOutput->AddTexte('D�tail des paiements');
    $MainOutput->br();
...
$MainOutput->CloseRow();
	...
	foreach($payments as $id_payment => $payment){
        ...
        if(abs($payment_unbalance)> 0.01){
            $MainOutput->AddTexte("<span class=Warning>D�balance: ". number_format($payment_unbalance,2)."</span>");
        }
