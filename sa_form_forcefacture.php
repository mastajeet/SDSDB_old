...
if(isset($_POST['FORMActionRadio']) and $_POST['FORMActionRadio']=="GenerateFull"){
    ...
    $MainOutput->Addform('Shift n\'ayant pas été facturé','index.php?Section=SuperAdmin&ToDo=Force_Facture');
    $Radio = array();
    while($Rep = $SQL->FetchArray()){
        ...
        $SQL2->Select($Req2);
        $Desc .= " - <a href=index.php?Section=Display_Shift&Semaine=".$Rep['Semaine']." target=_BLANK1>".$EndDate['Start']."</a> (".$Rep['nb']." Shifts non facturés)";
    ...
    $ActionRadio = array('Marquer shift comme facturés'=>'Mark','Générer la facture totale'=>'GenerateFull');
    //$ActionRadio = array('Marquer shift comme facturés'=>'Mark','Générer une facture partielle'=>'GenerateOnly','Générer la facture totale'=>'GenerateFull');
