...
if(isset($_GET['Semaine']) && !isset($_POST['FORMGenerateCote'])){
	...
	$Req = "SELECT Cote, sum(Facture), FrequenceFacturation as a FROM shift LEFT JOIN installation JOIN client on client.IDClient = installation.IDClient and shift.IDInstallation = installation.IDInstallation WHERE `Semaine`=".$_GET['Semaine']." GROUP BY installation.Cote ORDER BY installation.Cote ASC";
	...
	$MainOutput->Opencol();
		...
		$MainOutput->InputRadio('GenerateCote',$Installation. "(".$Frequence[$Rep['FrequenceFacturation']].")",'','Piscine','VER');
		$MainOutput->inputhidden_env('FrequenceFacturation',$Rep['FrequenceFacturation']);
...
//IL FAUT CHÉKER SI LES HEURES ONT PAS DÉJA ÉTÉ FACTUÉES

if($_POST['FrequenceFacturation']="H"){
...
$i=0;
	$InSemaine = $_POST['Semaine'];
while($i<=2){
	$_POST['Semaine'] = get_last_sunday(1,$_POST['Semaine']);	
	$InSemaine .= "," & $_POST['Semaine'];
	$i++;
}
	
	$Req = "SELECT sum(Facture) as a FROM shift LEFT JOIN installation on shift.IDInstallation = installation.IDInstallation WHERE `Semaine` IN (".$InSemaine.") AND installation.Cote='".$_POST['FORMCote']."' GROUP BY Cote";
...
$Rep = $SQL->FetchArray();
if($Rep[0]>0){
	$MainOutput->AddTexte('La facture pour cette période est déjà faite','Warning');
}elseif(get_last_sunday(-3,$_POST['Semaine'])>=get_last_sunday(0,time())){
	$MainOutput->AddTexte('Vous ne pouvez pas faire la facturation pour une période non complétée','Warning');
}else{
	$SQLins = new sqlclass();
